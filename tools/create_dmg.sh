#!/bin/bash
# Create DMG with custom background for SoundBridge
...
echo -e "${BLUE}----------------------------------------${NC}"
echo -e "${BLUE}  Creating SoundBridge DMG${NC}"
echo -e "${BLUE}----------------------------------------${NC}"

# Check if app bundle exists
if [ ! -d "$APP_BUNDLE" ]; then
    echo "ERROR: SoundBridge.app not found at $APP_BUNDLE"
...
if ! command -v create-dmg &> /dev/null; then
    ...
    cp -R "$APP_BUNDLE" "$PROJECT_ROOT/dist/dmg_temp/SoundBridge.app"
    ln -s /Applications "$PROJECT_ROOT/dist/dmg_temp/Applications"
    rm -f "$DMG_PATH"
    hdiutil create -volname "SoundBridge" -srcfolder "$PROJECT_ROOT/dist/dmg_temp" -ov -format UDZO -fs HFS+ "$DMG_PATH"
    rm -rf "$PROJECT_ROOT/dist/dmg_temp"
...
create-dmg \
    ...
    --window-size 660 450 \
    --icon-size 100 \
    --icon "SoundBridge.app" 210 220 \
    --app-drop-link 455 220 \
    --hide-extension "SoundBridge.app" \
    --no-internet-enable \
    "$DMG_PATH" \
    "$APP_BUNDLE"
