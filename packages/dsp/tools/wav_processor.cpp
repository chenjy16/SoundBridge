...
void createBassBoostPreset(soundbridge_preset_t* preset) {
    ...
    preset->bands[1].type = SOUNDBRIDGE_FILTER_PEAK;
    ...
    preset->bands[2].type = SOUNDBRIDGE_FILTER_PEAK;

    preset->preamp_db = -6.0f; // Reduce preamp to prevent clipping
    preset->limiter_enabled = true;
}

void createTrebleBoostPreset(soundbridge_preset_t* preset) {
    soundbridge_dsp_preset_init_flat(preset);
    ...
    preset->bands[0].type = SOUNDBRIDGE_FILTER_PEAK;
    ...
    preset->bands[3].type = SOUNDBRIDGE_FILTER_HIGH_SHELF;

    preset->preamp_db = -8.0f; // Heavy preamp reduction
    preset->limiter_enabled = true;
}

void createVocalEnhancePreset(soundbridge_preset_t* preset) {
    soundbridge_dsp_preset_init_flat(preset);
    ...
    preset->bands[0].q_factor = 0.707f;
    preset->bands[0].type = SOUNDBRIDGE_FILTER_HIGH_PASS;
...
int main(int argc, char* argv[]) {
    if (argc < 3) {
        std::cout << "SoundBridge DSP WAV Processor" << std::endl;
    ...
    // Create DSP engine
    soundbridge_dsp_engine_t* engine = soundbridge_dsp_create(header.sample_rate);
    if (!engine) {
    ...
    soundbridge_error_t err = soundbridge_dsp_apply_preset(engine, &preset);
    ...
    soundbridge_dsp_process_interleaved(engine, samples.data(), samples.data(), num_frames);

    std::cout << "Processed " << num_frames << " frames" << std::endl;

    // Get statistics
    soundbridge_stats_t stats;
    soundbridge_dsp_get_stats(engine, &stats);
